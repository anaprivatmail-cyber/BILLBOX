...
import type { Bill, BillStatus, CreateBillInput, UpdateBillInput } from './types'
...
export function isOverdue(bill: Bill): boolean {
  ...
  return due.getTime() < new Date(today.toDateString()).getTime() // compare dates ignoring time
}
